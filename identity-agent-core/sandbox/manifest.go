...
func (m *AppManifest) Validate() error {
	...
	if m.ExecutionType != "docker" && m.ExecutionType != "compiled" {
		return fmt.Errorf("execution_type must be 'docker' or 'compiled', got '%s'", m.ExecutionType)
	}
	...
	if m.ExecutionType == "docker" && m.Docker == nil {
		return fmt.Errorf("docker config is required when execution_type is 'docker'")
	}
...
func (m *AppManifest) IsDocker() bool {
	return m.ExecutionType == "docker"
}
...
func (m *AppManifest) DisplayPort() string {
	...
	for port, role := range m.Docker.Ports {
		if role == "display" {
			return port
		}
	}
