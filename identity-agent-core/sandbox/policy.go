...
func (pe *PolicyEngine) CheckDomain(instanceID, appID, domain, method, urlStr string) (action string, rule string) {
	...
	if pe.tracer != nil && pe.tracer.IsEnabled() {
		pe.tracer.Emit("policy", "check_start", "egress", appID, instanceID,
			fmt.Sprintf("Evaluating domain: %s", domain),
			map[string]interface{}{"domain": domain, "method": method, "url": urlStr, "has_manifest": hasManifest})
	}
	...
	if pe.tracer != nil && pe.tracer.IsEnabled() {
		rulesSummary := make([]map[string]string, 0, len(userRules))
		for _, r := range userRules {
			rulesSummary = append(rulesSummary, map[string]string{"type": r.RuleType, "target": r.Target, "source": r.Source})
		}
		pe.tracer.Emit("policy", "rules_loaded", "egress", appID, instanceID,
			fmt.Sprintf("Loaded %d user rules", len(userRules)),
			map[string]interface{}{"rules": rulesSummary})
	}
	...
	for _, r := range userRules {
		if r.RuleType == "domain_block" && MatchDomain(r.Target, domain) {
			...
			if pe.tracer != nil && pe.tracer.IsEnabled() {
				pe.tracer.Emit("policy", "decision", "egress", appID, instanceID,
					fmt.Sprintf("BLOCKED by user rule: %s", r.Target),
					map[string]interface{}{"action": "auto_blocked", "rule_type": "user_block", "pattern": r.Target, "domain": domain})
			}
	...
	for _, r := range userRules {
		if r.RuleType == "domain_allow" && MatchDomain(r.Target, domain) {
			...
			if pe.tracer != nil && pe.tracer.IsEnabled() {
				pe.tracer.Emit("policy", "decision", "egress", appID, instanceID,
					fmt.Sprintf("APPROVED by user rule: %s", r.Target),
					map[string]interface{}{"action": "auto_approved", "rule_type": "user_allow", "pattern": r.Target, "domain": domain})
			}
	...
	if hasManifest {
		...
		if explicit && !allowed {
			...
			if pe.tracer != nil && pe.tracer.IsEnabled() {
				pe.tracer.Emit("policy", "decision", "egress", appID, instanceID,
					fmt.Sprintf("BLOCKED by manifest for %s", domain),
					map[string]interface{}{"action": "auto_blocked", "rule_type": "manifest_block", "domain": domain})
			}
		...
		if explicit && allowed {
			...
			if pe.tracer != nil && pe.tracer.IsEnabled() {
				pe.tracer.Emit("policy", "decision", "egress", appID, instanceID,
					fmt.Sprintf("APPROVED by manifest for %s", domain),
					map[string]interface{}{"action": "auto_approved", "rule_type": "manifest_allow", "domain": domain})
			}
	...
	if pe.tracer != nil && pe.tracer.IsEnabled() {
		pe.tracer.Emit("policy", "decision", "egress", appID, instanceID,
			fmt.Sprintf("HELD for operator: %s (unknown domain)", domain),
			map[string]interface{}{"action": "held", "domain": domain, "reason": "unknown_domain"})
	}
