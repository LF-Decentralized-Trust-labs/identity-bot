...
func NewManager(cfg ManagerConfig) (*Manager, error) {
	...
	proxyMgr, err := NewProxyManager(ProxyManagerConfig{
		// An ephemeral port is right for a proxy only local processes use:
		// nothing has to know it in advance. A workspace on another machine
		// does — its configuration is written once, at provisioning, and a
		// port that changes on every restart makes that configuration wrong
		// the next time the service comes up. So it can be pinned, and the
		// default stays ephemeral for anyone who does not need it.
...
// Proxy exposes the forward proxy, so the server layer can serve its certificate
// authority to callers that must trust it before they can make a request.
func (m *Manager) Proxy() *ProxyManager {
	return m.proxy
}
...
// proxyListenAddr resolves where the forward proxy listens.
//
// SANDBOX_PROXY_ADDR pins it, e.g. "0.0.0.0:8899". Unset keeps the previous
// behaviour of an ephemeral port on all interfaces.
//
// Pinning is opt-in rather than the default because a fixed, externally
// reachable port is a standing target, and most deployments have no caller that
// needs to know it ahead of time. The one that does is a workspace configured
// once and expected to keep working across restarts.
func proxyListenAddr() string {
	if addr := strings.TrimSpace(os.Getenv("SANDBOX_PROXY_ADDR")); addr != "" {
		log.Printf("[sandbox-proxy] Listening on the configured address %s "+
			"(SANDBOX_PROXY_ADDR); remote callers can be pointed here durably", addr)
		return addr
	}
	return "0.0.0.0:0"
}
