...
func TestPairingDisclosesNothingBeyondThePairwiseOffer(t *testing.T) {
	...
	// box to whoever asked first. A guard that blesses the thing it
...
// box is now told what to expect by whoever provisioned it, before it can be
// reached, and mints nothing itself.
...
func TestABoxNobodyToldRefusesEveryClaim(t *testing.T) {
...
//
// Whoever provisioned the box says this while it is still reachable only from
// the host that started it. A caller arriving later, from anywhere else, must
// not be able to point it at a claim of their own.
func TestOnlyTheFirstClaimExpectationIsAccepted(t *testing.T) {
	resetExpectedClaimForTest()
	if err := SetExpectedClaim("TOKEN-FROM-THE-PROVISIONER", "EIntendedOwner"); err != nil {
	...
	if !told || token != "TOKEN-FROM-THE-PROVISIONER" || owner != "EIntendedOwner" {
