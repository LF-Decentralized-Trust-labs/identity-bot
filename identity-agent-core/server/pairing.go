...
func (s *CoreServer) handlePairingComplete(w http.ResponseWriter, r *http.Request) {
	...
	if !told {
		// Nobody told this instance what to expect, so it cannot know a real
		// claim from a stranger's. Refusing is the safe direction: a box nobody
		// can claim is recoverable, a box anybody can claim is not.
	...
	// prevent -- and the whole point of binding a claim to a buyer.
	if subtle.ConstantTimeCompare([]byte(expectedOwner), []byte(req.OwnerAID)) != 1 {
		writeError(w, http.StatusForbidden, "Wrong owner",
			"this instance was provisioned to answer to a different identity than the one claiming it")
		return
	}
