# Project Context — identity-bot

Guidance for AI coding agents working in this repository (Claude Code reads it as `CLAUDE.md`, Codex as `AGENTS.md`). This is the single canonical source — the per-tool files in the repo root are generated from it by the `dotfiles` installer, so never edit them directly; edit this file (or run the installer's `capture` step to pull in-session edits back here).

## Planning Documents (check these at the start of every session)

All planning docs live in `/Users/robandersen/Documents/GitHub/strategy/` (also at `https://github.com/bobert600/strategy`).
...
| `3. Docs/` | `architecture-doctrines.md` | **Canonical architectural doctrines (D1–Dn)** — cross-cutting principles every ARCH doc must cite. Consult before every architecture decision. Never add or modify doctrines without explicit user approval. |
| `3. Docs/` | `shared-modules.md` | **Catalog of app-wide reusable modules/components** (e.g., Share modal). When a capability is used by multiple features, use or extend the shared module rather than reinventing. Never add new modules without explicit user approval. |
...
### Core Concept: Two Topologies, Four Configurations

Every running Identity Agent instance is in exactly one of **two topologies**. This model from ADR-006 is the authoritative terminology (it supersedes the "3 topological states × 2 device types" and "four modes" language in older ADRs).
...
In either topology, the **computer** can be the user's own (laptop, desktop, mini-PC) or a **black box computer** — professionally managed in a data center, sealed via TEE attestation so even the operator can't read into it. Internal/architecture term: **black box infrastructure** (architecture-doctrines.md, D10). "Black box computer" is the consumer-facing noun for one user's instance.

**Four Launch Configurations:**

| # | Topology | Computer | Who it suits |
|---|---|---|---|
| 1 | Phone + Computer | **Black box computer** (data center) | **Most common at launch.** 60-second setup, no hardware to maintain, always-on. The default we recommend. |
...
**Future:** Personal black box infrastructure (single- or multi-tenant TEE hardware operated by you, family member, neighbor, or community steward — same protocol, different operator). M9 Backup is orthogonal — any configuration can mirror to a second computer.

**Two Device Types** (engineering view):

| Device | KERI Engine | Backend Engine |
|---|---|---|
| Computer (Linux/macOS/Windows) | Go backend → Python `keripy` (child process) | Go Core (local binary, port 5050) |
| Phone (iOS/Android) | Rust bridge via `flutter_rust_bridge` FFI | Go Core via `gomobile` (embedded, port 8642) |

**Configuration → `KeriService` Implementation:**

| Configuration | Phone runs | Computer runs |
|---|---|---|
| Phone + Computer (own or black box) | `MobileOnDeviceKeriService(pairedServerUrl:)` — keys local, paired with backend | `DesktopOnDeviceKeriService` — provides backend, no local keys |
| Computer only (own or black box) | — | `DesktopOnDeviceKeriService` — full standalone (keys + backend local) |

In Phone + Computer, when the identity was created on the computer first (e.g., user adds a phone later), the phone uses `MobileRemoteKeriService(serverUrl:)` as a thin controller (no local keys). The phone-vs-computer key-residence is auto-detected from creation order.

### Critical Invariant

**Stateful KERI operations always use the LOCAL engine** in every configuration. The remote server is ONLY used for backend services (persistence, OOBI, contacts, tunneling) and stateless KERI operations (format-credential, resolve-oobi, generate-multisig-event). **The remote server never performs stateful KERI operations on behalf of the local device.**
...
### Mobile (iOS / Android)
- `codemagic.yaml` here builds: `android-release` (debug-signed arm64 **sideload APK**, no store upload), `ios-release` (signed IPA → TestFlight), plus desktop workflows.
- **Play Store / App Store publishing for Grape ID is NOT in this repo.** It lives in the separate private **`GrapeID/release-pipeline`** repo, which clones this public source, applies a release-signing patch to `identity_agent_ui/android/app/build.gradle` at build time, and produces a signed `.aab`. This keeps Grape-ID store config out of the public LF repo. Full strategy: `strategy/3. Docs/release-automation-strategy.md`.
- Rust cross-compilation and `gomobile` binding only run in CI, not locally.

## Commit Requirements

All commits must include a DCO (Developer Certificate of Origin) sign-off. Append this line to every commit message — **the email MUST be in angle brackets** or the DCO bot rejects it ("Sign-off not found"):

```
Signed-off-by: Rob Andersen <rob@antispamguy.org>
```

**Rob is the sole sign-off and author — NEVER add `Co-Authored-By: Claude` (or any AI co-author) to commits.** The Linux Foundation requires the responsible human to sign off, not the AI agent. A `Co-Authored-By` line also breaks DCO (the co-author has no matching sign-off). This applies to ALL repos (public LF + private Grape ID), for consistency.
