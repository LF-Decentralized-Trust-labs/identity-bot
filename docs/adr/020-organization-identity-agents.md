...
- **Org profile field specification** *(task required before first org demo)*: The current open source org profile includes `org_name`, `org_type`, and `jurisdiction`. "Jurisdiction" in particular needs precise definition — legal jurisdiction of incorporation? Operating jurisdiction? Both? What format (ISO country code, US state code, free text)? What other fields are universally applicable vs. industry-specific? This must be resolved before the org onboarding flow is built. Tracked in `tasks.md`.
