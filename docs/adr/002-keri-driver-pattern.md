...
On mobile OS (iOS/Android), Python cannot run. The KERI driver is **optional** —
the Go Core's `ServerConfig` has an `EnableKeriDriver` flag that is set to `false`
on mobile. Mobile devices use the Rust bridge for KERI operations instead (see
ADR 003 and ADR 004).
...
### 5. Optional Driver for Mobile

The Go Core server package (`identity-agent-core/server/`) accepts a `ServerConfig`
struct. When `EnableKeriDriver` is `false` (the default for mobile builds via
`mobilecore` package), the server:
- Does NOT spawn a Python process
- Does NOT register KERI proxy endpoints (`/api/inception`, `/api/rotation`, etc.)
- Still provides storage endpoints (`/api/store/identity`, `/api/store/event`),
  OOBI serving, contact management, and tunneling
- Returns helpful error messages if KERI endpoints are called, directing to use
  the Rust bridge instead

### 6. Driver API — Source of Truth for Naming
...
- **Mobile-ready:** The driver is optional. On mobile, the Go Core runs without
  the Python driver, and the Rust bridge handles KERI crypto instead.
...
- **Hard dependency:** If keripy or libsodium is unavailable, the entire identity
  system is non-functional on desktop. There is no degraded mode.
...
- `identity-agent-core/server/server.go` — Extracted server package with optional KERI driver support (`ServerConfig.EnableKeriDriver`)
- `identity-agent-core/main.go` — Desktop entry point (enables KERI driver)
- `identity-agent-core/mobilecore/mobilecore.go` — Mobile entry point (disables KERI driver)
