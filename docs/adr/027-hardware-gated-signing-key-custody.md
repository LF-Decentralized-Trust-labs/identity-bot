...
- No interaction with backup/recovery: the backup archive's encryption key is derived from the portable root seed (see the M09/M10 architecture), not from this device-local hardware wrap — a device-bound key was explicitly considered and rejected as a backup mechanism there, for the same reason it would defeat cross-device recovery. **Implementation note for whoever builds M09/M10:** the backup export step must call `SecureKeyStore.loadMnemonic()` (which unwraps) to obtain the plaintext mnemonic for the archive — never read the raw stored value directly, which after this ADR may be the `hw1:`-prefixed wrapped ciphertext rather than the mnemonic itself.
